...
# --- ১. কনফিগারেশন (আপনার সব আইডি ও টোকেন) ---
...
def admin_menu():
    markup = telebot.types.ReplyKeyboardMarkup(row_width=2, resize_keyboard=True)
    markup.add("📊 Total Users", "📜 User Full Details", "🏠 মেইন মেনু")
    return markup
...
def start(message):
    ...
    if not is_joined(message.chat.id):
        ...
        bot.send_message(uid, "<b>⚠️ জয়েন বাধ্যতামূলক!</b>\nচ্যানেল ও গ্রুপে জয়েন করে 'I join' বাটনে ক্লিক করুন।", reply_markup=markup, parse_mode="HTML")
        return

    # নম্বর ও লোকেশন কালেকশন (যদি না থাকে)
    if users[uid]["phone"] == "N/A":
        ...
        bot.send_message(uid, "🌟 <b>S.R.M TELECOM</b> বটের মেইন মেনু:", reply_markup=main_menu(message.chat.id), parse_mode="HTML")
...
# --- ৭. এডমিন রিপ্লাই লজিক (১০০% ফিক্সড) ---
...
# --- ৯. সাব-বাটন ও সমস্যা কালেকশন ফ্লো ---
...
def handle_all(message):
    ...
    # বাটন লজিক
    if text == "📲 Apps link":
        bot.send_message(uid, "🔗 <b>অ্যাপ লিঙ্ক:</b> https://play.google.com/store/apps/details?id=com.shuvotelecom24.user", parse_mode="HTML")
    elif text == "💰 Balance Problem": bot.send_message(uid, "মাধ্যম সিলেক্ট করুন:", reply_markup=balance_menu())
    elif text == "⚡ Recharge Problem": bot.send_message(uid, "অপারেটর সিলেক্ট করুন:", reply_markup=recharge_menu())
    elif text == "🚗 Drive Problem": bot.send_message(uid, "অপারেটর সিলেক্ট করুন:", reply_markup=drive_menu())
    ...
    # সাব-বাটন চেক
    elif text and any(x in text for x in ["Recharge", "Drive", "Personal", "Agent"]):
        ask_problem(message, text)

    # ফরওয়ার্ডিং (ইউজার মেসেজ পাঠালে এডমিন পাবে)
    elif message.chat.id not in ADMINS:
        for admin in ADMINS:
            if message.content_type == 'text':
                bot.send_message(admin, f"📩 <b>বার্তা:</b>\n{message.text}\n\n🆔 ID: <code>{uid}</code>", parse_mode="HTML")
            elif message.content_type == 'photo':
                bot.send_photo(admin, message.photo[-1].file_id, caption=f"📸 <b>ফটো আসিয়াছে</b>\n\n🆔 ID: <code>{uid}</code>", parse_mode="HTML")
        bot.send_message(uid, "✅ আপনার বার্তা এডমিনকে জানানো হয়েছে।")
